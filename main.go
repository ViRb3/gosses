...
func handleContent(c echo.Context) error {
	filePath := resolvePrefixedPath(c.Request().URL.Path)
...
func handleZip(c echo.Context) error {
	...
	zipFullPath := resolvePrefixedPath(zipPath)
...
// Check resolvePath.
func resolvePrefixedPath(unsafePath string) string {
...
// Resolves file paths relative to the rootPath.
